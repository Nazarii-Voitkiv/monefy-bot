...
2. Створіть файл `.env`:
   ...
   # за потреби додайте ALLOWED_USER_IDS (список ID через кому)
   ```
...
> ⚠️ Безкоштовний тариф exchangerate-api.com дозволяє ~1500 запитів/місяць. Кеш у БД гарантує, що кожна дата запитується лише один раз. Якщо треба обмежити доступ — встанови `ALLOWED_USER_IDS=489177683` (можна кілька через кому).
